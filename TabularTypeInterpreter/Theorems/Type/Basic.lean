...
theorem soundness (σee : [[Γc; Γ ⊢ σ₀ ≡ σ₁ ⇝ F]]) (Γwe : [[Γc ⊢ Γ ⇝ Δ]])
  ...
  --   -- TODO: Is this actually possible since we don't know what the row may be (i.e. it may be a
  --   -- variable in which case the listApp equivalence rule wouldn't apply).
  --   exact .equiv (this .id) sorry
  -- | sumRow ρ₀₁ee sumke =>
  --   let Alc := σ₀ke.soundness Γwe κe |>.TypeVarLocallyClosed_of
  --   let .sum _ ρ₀ke := σ₀ke
  --   let .sum _ ρ₁ke := σ₁ke
  ...
  --         · case tail mem'''' => nomatch mem''''
  | tc τ₀₁ee σ'op₀₁ee γcin TCₛee TCτ₀ke τ₀₁ih σ'op₀₁ih TCₛih =>
    rename TypeScheme => σ'
    ...
    · case tail mem' =>
      ...
      rw [List.get!_cons_succ, List.map_singleton_flatten, ← Range.map, Range.get!_map mem.upper,
