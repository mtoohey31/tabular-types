...
instance : Inhabited «Type» where
  default := .list []
in
theorem to_Kinding (Mte : [[Γᵢ; Γc; Γ ⊢ M : σ ⇝ E]]) (Γwe : [[Γc ⊢ Γ ⇝ Δ]])
  : ∃ A, [[Γc; Γ ⊢ σ : * ⇝ A]] := by
  induction Mte using rec (motive_2 := fun Γᵢ Γc Γ ψ E ψce => ∃ A, [[Γc; Γ ⊢ ψ : C ⇝ A]])
    generalizing Δ with
  ...
  | lam I _ τ₀ke ih =>
    ...
    let ⟨_, τ₁ke⟩ := ih x xninI <| Γwe.termExt xninΓ τ₀ke
    exact ⟨_, τ₀ke.arr <| τ₁ke.TermVar_drop (Γ' := .empty)⟩
  | app _ _ φih τih =>
    let ⟨_, .arr _ τ₁ke⟩ := φih Γwe
    exact ⟨_, τ₁ke⟩
  | qualI I ψke _ ih =>
    rename TypeEnvironment => Γ
    let ⟨x, xnin⟩ := I ++ Γ.termVarDom |>.exists_fresh
    let ⟨xninI, xninΓ⟩ := List.not_mem_append'.mp xnin
    let ⟨_, γke⟩ := ih x xninI <| Γwe.constrExt xninΓ ψke
    exact ⟨_, ψke.qual (γke.Constr_drop (Γ' := .empty)) .star⟩
  | qualE ψce _ _ γih =>
    let ⟨_, .qual _ γke _⟩ := γih Γwe
    exact ⟨_, γke⟩
  | schemeI I _ κe ih =>
    rename TypeScheme => σ'
    rename TypeEnvironment => Γ
    ...
    let ⟨_, σ'ke⟩ := ih a aninI Γawe
    let Alc := σ'ke.soundness Γawe .star |>.TypeVarLocallyClosed_of
    ...
    exact ⟨_, .scheme (I ++ ↑a :: Γ.typeVarDom) (by
      ...
      have := σ'ke'.Monotype_open_preservation Γa'awe (nomatch ·) aninσ'
  ...
  | schemeE _ τke σih =>
    ...
    exact ⟨_, σ'opke'.Monotype_open_preservation Γawe (nomatch ·) aninσ' aninA τke (Γ' := .empty)⟩
  | «let» I _ σ₀ke _ _ σ₁ih =>
    rename TypeEnvironment => Γ
    let ⟨x, xnin⟩ := Γ.termVarDom ++ I |>.exists_fresh
    let ⟨xninΓ, xninI⟩ := List.not_mem_append'.mp xnin
    let ⟨_, σ₁ke⟩ := σ₁ih _ xninI <| Γwe.termExt xninΓ σ₀ke
    exact ⟨_, σ₁ke.TermVar_drop (Γ' := .empty)⟩
  | annot _ ih => exact ih Γwe
  ...
  | member γcin _ ih =>
    ...
    exact ⟨_, σ'ke'.Monotype_open_preservation Γawe (nomatch ·) aninσ' aninA τke (Γ' := .empty)⟩
  | «order» _ ih =>
    rename ProdOrSum => Ξ
    match Ξ with
    | .prod =>
      let ⟨_, .prod _ ρke⟩ := ih Γwe
      exact ⟨_, .prod .comm ρke⟩
    | .sum =>
      let ⟨_, .sum _ ρke⟩ := ih Γwe
  ...
  | «ind» Iₘ Iₛ ρke τke κe =>
    ...
    exact ⟨_, τke'.Monotype_open_preservation Γawe (nomatch ·) aninτ aninB ρke (Γ' := .empty)⟩
  | «splitₚ» _ _ prodih splitih =>
    let ⟨_, .prod μke ρ₂ke⟩ := prodih Γwe
    let ⟨_, .split concatke⟩ := splitih
    let .concat _ ρ₀ke ρ₁ke ρ₂ke' _ _ _ (A₀ := A₀) (A₁ := A₁) := concatke
    let ⟨κeq, _⟩ := ρ₂ke.deterministic ρ₂ke'
    cases Kind.row.inj κeq
    apply Exists.intro _
    apply TypeScheme.KindingAndElaboration.prod .comm
    have : none = Option.filter (fun _ => false) (some Kind.star) := by
      rw [Option.filter, if_neg (nomatch ·)]
    rw [← Range.map_get!_eq (as := [_, _]), Range.map, ← List.map_singleton_flatten, ← Range.map,
        this]
    ...
    · rw [Range.map, List.map_singleton_flatten, Range.map_eq_of_eq_of_mem (by
      ...
      ), ← List.map_singleton_flatten, ← Range.map, List.length_cons, List.length_singleton]
  ...
  | «splitₛ» _ _ _ _ _ arrρ₁ih splitih =>
    let ⟨_, .arr (.sum μke ρ₁ke) τ₁ke⟩ := arrρ₁ih Γwe
    let ⟨_, .split concatke⟩ := splitih
...
theorem soundness (Mte : [[Γᵢ; Γc; Γ ⊢ M : σ ⇝ E]]) (σke : [[Γc; Γ ⊢ σ : * ⇝ A]])
  ...
  induction Mte using rec
    (motive_2 := fun Γᵢ Γc Γ ψ E ψce => ∀ {A Δ}, [[Γc; Γ ⊢ ψ : C ⇝ A]] → [[Γc ⊢ Γ ⇝ Δ]] → [[Δ ⊢ E : A]])
    generalizing Δ A with
  | var xinΓ => exact .var Γwe.soundness <| Γwe.TermVarIn_preservation xinΓ σke
  | lam I Mte τ₀ke ih =>
    ...
    exact ih _ xninI τ₁ke' Γxwe
  | app Mte _ Mih Nih =>
    let ⟨_, arrke@(.arr τ₀ke τ₁ke)⟩ := Mte.to_Kinding Γwe
    rcases σke.deterministic τ₁ke with ⟨_, rfl⟩
    exact .app (Mih arrke Γwe) (Nih τ₀ke Γwe)
  | qualI I ψke _ ih =>
    ...
    exact ih _ xninI γke' Γxwe
  | qualE ψce Mte ψih qualih =>
    let ⟨_, qualke@(.qual ψke γke _)⟩ := Mte.to_Kinding Γwe
    rcases σke.deterministic γke with ⟨_, rfl⟩
    exact .app (qualih qualke Γwe) (ψih ψke Γwe)
  | schemeI I _ κe ih =>
    ...
    exact ih _ aninI (σ'ke _ aninI') (Γwe.typeExt aninΓ κe)
  | schemeE Mte τke ih =>
    let ⟨_, schemeke@(.scheme I σ'ke κ₀e)⟩ := Mte.to_Kinding Γwe
    rename TypeEnvironment => Γ
    rename TypeScheme => σ'
    rename «Type» => A
    ...
    let Γawe := Γwe.typeExt aninΓ κ₀e
    let σke' := σ'ke a aninI |>.Monotype_open_preservation Γawe (nomatch ·) aninσ' aninA τke
      (Γ' := .empty)
    rcases σke.deterministic σke' with ⟨_, rfl⟩
    exact .typeApp (ih schemeke Γwe) (τke.soundness Γwe κ₀e)
  | «let» I Mte σ₀ke Nte Mih Nih =>
    apply Typing.app _ <| Mih σ₀ke Γwe
    rename TypeEnvironment => Γ
    apply Typing.lam <| I ++ Γ.termVarDom
    intro x xnin
    let ⟨xninI, xninΓ⟩ := List.not_mem_append'.mp xnin
  ...
  | unlabelProd Mte _ ih =>
    ...
    apply Typing.prodElim _ ⟨Nat.le.refl, Nat.one_pos⟩ (A := fun _ => A)
    rw [List.map_singleton_flatten, Range.toList, if_neg (nomatch ·), if_pos Nat.one_pos,
        Range.toList, if_neg (nomatch ·), Nat.zero_add, if_neg (Nat.not_lt_of_le Nat.le.refl),
        List.map_singleton]
    exact ih prodke Γwe
  | unlabelSum Mte _ τke ih =>
    let ⟨_, sumke@(.sum _ rowke)⟩ := Mte.to_Kinding Γwe
    rcases rowke.singleton_row_inversion with ⟨_, _, κeq, _, rfl, τke'⟩
    cases Kind.row.inj κeq
    rcases σke.deterministic τke with ⟨_, rfl⟩
    rcases σke.deterministic τke' with ⟨_, rfl⟩
    ...
    conv => simp_match
    exact .id Γwe.soundness <| τke.soundness Γwe .star
  | «prj» Mte _ Mih containih =>
    let ⟨_, prodke@(.prod μke ρ₀ke)⟩ := Mte.to_Kinding Γwe
    apply Typing.app _ <| Mih prodke Γwe
    let .prod _ ρ₁ke := σke
    let Fty := containih (.contain μke ρ₁ke ρ₀ke .star) Γwe
    rw [← Range.map_get!_eq (as := [_, _]), Range.map, ← List.map_singleton_flatten,
        ← Range.map] at Fty
    let πty := Fty.prodElim ⟨Nat.le.refl, Nat.two_pos⟩
  ...
  | concat Mte Nte ψce Mih Nih concatih =>
    let ⟨_, prod₀ke⟩ := Mte.to_Kinding Γwe
    let ⟨_, prod₁ke⟩ := Nte.to_Kinding Γwe
    apply Typing.app _ <| Nih prod₁ke Γwe
    apply Typing.app _ <| Mih prod₀ke Γwe
    let .prod μke ρ₀ke := prod₀ke
    let .prod _ ρ₁ke := prod₁ke
    let .prod _ ρ₂ke := σke
    let ⟨_, .concat ..⟩ := ψce.to_Kinding Γwe
    let Fty := concatih (.concat μke ρ₀ke ρ₁ke ρ₂ke .star (.contain μke ρ₀ke ρ₂ke .star)
      (.contain μke ρ₁ke ρ₂ke .star)) Γwe
    rw [← Range.map_get!_eq (as := [_, _, _, _]), Range.map, ← List.map_singleton_flatten,
        ← Range.map] at Fty
    let πty := Fty.prodElim ⟨Nat.le.refl, Nat.le.refl.step.step.step⟩
    ...
    simp [Type.Type_open] at this
    rw [ρ₀ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of.Type_open_id,
        ρ₁ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of.Type_open_id,
        ρ₂ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of.Type_open_id] at this
    exact .equiv this <| .arr (.prod .listAppIdL) (.arr (.prod .listAppIdL) (.prod .listAppIdL))
  | «inj» Mte _ Mih containih =>
    let ⟨_, sumke@(.sum μke ρ₀ke)⟩ := Mte.to_Kinding Γwe
    apply Typing.app _ <| Mih sumke Γwe
    let .sum _ ρ₁ke := σke
    let Fty := containih (.contain μke ρ₀ke ρ₁ke .star) Γwe
    rw [← Range.map_get!_eq (as := [_, _]), Range.map, ← List.map_singleton_flatten,
        ← Range.map] at Fty
    let πty := Fty.prodElim ⟨Nat.le.refl.step, Nat.le.refl⟩
  ...
  | elim Mte Nte ψce τke Mih Nih concatih =>
    let ⟨_, arr₀ke⟩ := Mte.to_Kinding Γwe
    let ⟨_, arr₁ke⟩ := Nte.to_Kinding Γwe
    apply Typing.app _ <| Nih arr₁ke Γwe
    apply Typing.app _ <| Mih arr₀ke Γwe
    ...
    rw [← Range.map_get!_eq (as := [_, _, _, _]), Range.map, ← List.map_singleton_flatten,
        ← Range.map] at Fty
    let πty := Fty.prodElim ⟨Nat.le.refl.step, Nat.le.refl.step.step⟩
    rw [List.length_cons, List.length_cons, List.length_cons, List.length_singleton,
        List.get!_cons_succ, List.get!_cons_zero] at πty
    simp only at πty
    have := πty.typeApp .id (B := [[λ a : *. a$0]])
    simp [Type.Type_open] at this
    let A₀lc := ρ₀ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of
    let A₁lc := ρ₁ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of
    let A₂lc := ρ₂ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of
    rw [A₀lc.weaken (n := 1).Type_open_id, A₁lc.weaken (n := 1).Type_open_id,
        A₂lc.weaken (n := 1).Type_open_id] at this
    have := this.typeApp <| τke.soundness Γwe .star
    simp [Type.Type_open] at this
  ...
  | sub Mte τse ih =>
    let ⟨_, τ₀ke⟩ := Mte.to_Kinding Γwe
    let ⟨_, _, _, τ₀ke', τ₁ke⟩ := τse.to_Kinding Γwe
    rcases τ₀ke.deterministic τ₀ke' with ⟨rfl, rfl⟩
    rcases σke.deterministic τ₁ke with ⟨_, rfl⟩
    exact .app (τse.soundness Γwe τ₀ke τ₁ke .star) (ih τ₀ke Γwe)
  | member γcin TCce TCih =>
    rename_i A' _ _ _ _ _ _
    let ⟨_, tcke@(.tc Γcw γcin' τke)⟩ := TCce.to_Kinding Γwe
    ...
    rw [TypeEnvironment.empty_append] at Γawe σ'ke'
    let σke' := σ'ke'.Monotype_open_preservation Γawe (nomatch ·) aninσ' aninA' τke (Γ' := .empty)
    rcases σke.deterministic σke' with ⟨_, rfl⟩
    let Ety := TCih tcke Γwe
    rw [← Range.map_get!_eq (as := _ :: _), Range.map, ← List.map_singleton_flatten,
        ← Range.map] at Ety
    let πty := Ety.prodElim ⟨Nat.le.refl, by rw [List.length_cons]; exact Nat.succ_pos _⟩
    rw [List.length_cons, List.map_singleton_flatten, List.length_map, Range.length_toList,
        Nat.sub_zero, List.get!_cons_zero] at πty
    simp only at πty
    exact πty
  | «order» M'te ih =>
    rename ProdOrSum => Ξ
    ...
    | .prod =>
      let .prod _ ρke := σke
      exact ih (.prod .comm ρke) Γwe
    | .sum =>
      let .sum _ ρke := σke
      exact ih (.sum .comm ρke) Γwe
  | «ind» Iₘ Iₛ ρke τke κe Mte Nte indce Mih Nih indih =>
    ...
    rw [← QualifiedType.TypeVar_open, ← TypeScheme.TypeVar_open] at τke'
    let τopρke :=
      τke'.Monotype_open_preservation Γawe (nomatch ·) aninτ aninB ρke (Γ' := .empty)
    rcases τopρke.deterministic σke with ⟨_, rfl⟩
    let τopemptyke :=
      τke'.Monotype_open_preservation Γawe (nomatch ·) aninτ aninB .empty_row (Γ' := .empty)
    apply Typing.app _ <| Nih τopemptyke Γwe
    let ⟨_, indke@(.ind I₀ I₁ ρke' κe' keBᵣ keBₗ)⟩ := indce.to_Kinding Γwe
    rename_i Bᵣ Bₗ _ _ _
    rcases ρke.deterministic ρke' with ⟨κeq, rfl⟩
    cases Kind.row.inj κeq
    cases κe.deterministic κe'
    let Fty := indih indke Γwe
    have := Fty.typeApp <| .lam (Iₘ ++ Γ.typeVarDom) fun a anin =>
      let ⟨aninI, aninΓ⟩ := List.not_mem_append'.mp anin
      τke a aninI |>.soundness (Γwe.typeExt aninΓ κe.row) .star
    simp [Type.Type_open] at this
    rw [ρke.soundness Γwe κe.row |>.TypeVarLocallyClosed_of.Type_open_id] at this
    ...
    apply Mih _ aₗninIₛ _ aₜninIₛ _ aₚninIₛ _ aᵢninIₛ _ aₙninIₛ _ Γaₗₜₚᵢₙwe
    ...
    let Bᵣlc := keBᵣ'.soundness Γaₗₜₚᵢₙwe .constr |>.TypeVarLocallyClosed_of.weaken (n := 5)
    ...
    let Bₗlc := keBₗ'.soundness Γaᵢₙwe .constr |>.TypeVarLocallyClosed_of.weaken (n := 2)
      |>.TypeVar_open_drop Nat.le.refl.step
      |>.TypeVar_open_drop Nat.le.refl
    ...
    let Bopaₚlc := τopaₚke.soundness Γaₗₜₚᵢₙwe .star |>.TypeVarLocallyClosed_of
    ...
    let Bopaᵢlc := τopaᵢke.soundness Γaₗₜₚᵢₙwe .star |>.TypeVarLocallyClosed_of
  ...
  | «splitₚ» Mte splitce Mih splitih =>
    let ⟨_, .prod μke ρ₂ke⟩ := Mte.to_Kinding Γwe
    let ⟨_, splitke@(.split concatke)⟩ := splitce.to_Kinding Γwe
    ...
    intro EA mem
    conv => simp_match
    rw [Range.toList, if_neg (nomatch ·), if_pos (Nat.succ_pos _), List.map_cons, Range.toList,
        if_neg (nomatch ·), Nat.zero_add, if_pos (Nat.lt_succ_self _), List.map_cons, Range.toList, if_neg (nomatch ·)] at mem
    simp only at mem
    rw [if_neg (Nat.not_lt_of_le Nat.le.refl), List.map_nil] at mem
    let Fty := splitih splitke Γwe
    rw [← Range.map_get!_eq (as := [_, _, _, _]), Range.map, ← List.map_singleton_flatten,
        ← Range.map] at Fty
    cases mem
    · case head =>
      simp only
      apply Typing.app _ <| Mih (.prod μke ρ₂ke) Γwe
      let πty := Fty.prodElim ⟨Nat.le.refl.step.step, Nat.le.refl.step⟩
      rw [List.length_cons, List.length_cons, List.length_cons, List.length_singleton,
      ...
      rw [← Range.map_get!_eq (as := [_, _]), Range.map, ← List.map_singleton_flatten,
          ← Range.map] at πty
      let π'ty := πty.prodElim ⟨Nat.le.refl, Nat.le.refl.step⟩
      rw [List.length_cons, List.length_singleton, List.get!_cons_zero] at π'ty
      simp only at π'ty
      have := π'ty.typeApp .id (B := [[λ a : *. a$0]])
      simp [Type.Type_open] at this
      rw [liftke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of.Type_open_id,
          ρ₂ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of.Type_open_id] at this
      let mem : 0 ∈ [0:2] := ⟨Nat.le.refl, Nat.two_pos⟩
      let τ'ke' := τ'ke _ mem
      simp only at τ'ke'
    ...
    · case tail mem' =>
      ...
      simp only
      apply Typing.app _ <| Mih (.prod μke ρ₂ke) Γwe
      let πty := Fty.prodElim ⟨Nat.le.refl.step.step.step, Nat.le.refl⟩
      rw [List.length_cons, List.length_cons, List.length_cons, List.length_singleton,
      ...
      rw [ρ₁ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of.Type_open_id,
          ρ₂ke.soundness Γwe (.row .star) |>.TypeVarLocallyClosed_of.Type_open_id] at this
      let mem : 1 ∈ [0:2] := ⟨Nat.le.refl.step, Nat.one_lt_two⟩
      let τ'ke' := τ'ke _ mem
      simp only at τ'ke'
      rw [← And.right <| Prod.mk.inj <| Range.eq_of_mem_of_map_eq ξτseq _ mem] at τ'ke'
  ...
  | «splitₛ» Mte Nte splitce τ₁ke Mih Nih splitih =>
    let ⟨_, arr₁ke@(.arr sum₁ke τ₁ke')⟩ := Nte.to_Kinding Γwe
    rcases τ₁ke.deterministic τ₁ke' with ⟨_, rfl⟩
    let .sum _ ρ₁ke := sum₁ke
    apply Typing.app _ <| Nih arr₁ke Γwe
    let ⟨_, arr₀ke@(.arr sum₀ke τ₁ke'')⟩ := Mte.to_Kinding Γwe
    let .sum _ ρ₀ke := sum₀ke
    rcases τ₁ke.deterministic τ₁ke'' with ⟨_, rfl⟩
    ...
    rw [A₀lc.weaken (n := 1).Type_open_id, A₁lc.weaken (n := 1).Type_open_id,
        A₂lc.weaken (n := 1).Type_open_id] at this
    have := this.typeApp (τ₁ke.soundness Γwe .star)
    simp [Type.Type_open] at this
    rw [A₀lc.Type_open_id, A₁lc.Type_open_id, A₂lc.Type_open_id] at this
    exact .equiv this <| .arr (.arr (.sum .listAppIdL) .refl) <|
      .arr (.arr (.sum .listAppIdL) .refl) <| .arr (.sum .listAppIdL) .refl
