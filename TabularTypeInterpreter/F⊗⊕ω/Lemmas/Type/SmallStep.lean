...
theorem list (Asn : ∀ i ∈ [:n], [[Δ ⊢ SN(A@i)]])
  ...
  sorry
  -- suffices h : ∀ i < n,
  -- generalize Aseq : [:_].map _ = As, K?eq : Option.someIf .. = K? at st
  -- let .list A₁st (n := n') := st
  -- sorry
  ...
  --   constructor
  --   intro _ st
  --   nomatch Type.IsValue.not_step (.list (by nofun)) st
  -- | succ n' ih =>
  --   specialize Asn n' sorry
  --   generalize A'eq : A n' = A' at Asn
  --   induction Asn with
  --   | intro A'' _ ih' =>
  --     cases A'eq
  --     constructor
...
theorem list_inversion'
  : StronglyNormalizing Δ (Type.list As K?) → ∀ A ∈ As, [[Δ ⊢ SN(A)]] := sorry
...
theorem not_lam_listApp (A_not_mst_lam : ∀ Δ', Δ ≤ Δ' → ∀ K A', ¬[[Δ' ⊢ A ->* λ a : K. A']])
  (Asn : [[Δ ⊢ SN(A)]]) (Bsn : [[Δ ⊢ SN(B)]]) : [[Δ ⊢ SN(A ⟦B⟧)]] := by
  induction Asn generalizing B with
  | intro A' A'sn ihA =>
    induction Bsn with
    | intro B' B'sn ihB =>
      ...
      match st with
      | .listAppList .. =>
        rename_i K₁ _ n B' b _
        apply list
        intro i mem
        apply not_lam_app A_not_mst_lam (.intro _ A'sn) (.intro ..)
        intro _ st
        let st' := st.list (A₀ := B') (K := K₁) (b := b) (A₂ := fun j => B' (j + (i + 1))) (m := i)
          (n := n - (i + 1))
        rw (occs := .pos [4]) [← Nat.sub_self (i + 1)] at st'
        rw [Range.map_shift Nat.le.refl, ← Range.map_eq_cons_of_lt mem.upper,
            Range.map_append (Nat.zero_le _) mem.upper.le, ← Range.map] at st'
        exact list_inversion' (B'sn _ st') _ <| List.mem_append.mpr <| .inr <| .head _
      | .listAppId .. => exact .intro _ B'sn
      | .listAppComp .. => sorry
      | .listAppl A'st =>
        apply ihA _ A'st _ <| .intro _ B'sn
        intro Δ' le K A'' mst
        exact A_not_mst_lam Δ' le K A'' <| mst.head <| A'st.LE_weakening le
      | .listAppr B'st => exact ihB _ B'st
...
theorem IndexedStronglyNormalizing.not_lam_app
  ...
  (A'sn : [[Δ ⊢ SN(A')]]) (A'ki : [[Δ ⊢ A' : K₁ ↦ K₂]]) (B'isn : [[Δ' ⊢ SN K₁ (B')]]) (le : Δ ≤ Δ')
  : [[Δ' ⊢ SN K₂ (A' B')]] := by
  match K₂ with
  | [[*]] =>
    let A'B'ki := A'ki.LE_weakening le |>.app B'isn.to_Kinding
    refine ⟨A'B'ki, ?_⟩
    exact .not_lam_app A_not_mst_lam (A'sn.LE_weakening le A'ki) <| .of_Indexed B'isn
  | [[K₂₁ ↦ K₂₂]] =>
    let A'B'ki := A'ki.LE_weakening le |>.app B'isn.to_Kinding
    ...
    · intro B''isn
      apply IndexedStronglyNormalizing.not_lam_app _
        (.not_lam_app A_not_mst_lam (A'sn.LE_weakening le A'ki) (.of_Indexed B'isn)) A'B'ki B''isn
        le'
      intro Δ''' le''
  ...
  | [[L K₂']] =>
    ...
    refine ⟨
      A'B'ki,
      .not_lam_app A_not_mst_lam (A'sn.LE_weakening le A'ki) <| .of_Indexed B'isn,
      ?_
    ⟩
    intro _ _ _ mst
    suffices h : ∀ C, [[Δ' ⊢ A' B' ->* C]] → ∃ A'' B'', C = [[A'' B'']] by
...
theorem IndexedStronglyNormalizing.not_lam_listApp
  (A_not_mst_lam : ∀ Δ'', Δ' ≤ Δ'' → ∀ K A'', ¬[[Δ'' ⊢ A' ->* λ a : K. A'']])
  (A'sn : [[Δ ⊢ SN(A')]]) (A'ki : [[Δ ⊢ A' : K₁ ↦ K₂]]) (B'isn : [[Δ' ⊢ SN L K₁ (B')]])
  (le : Δ ≤ Δ') : [[Δ' ⊢ SN L K₂ (A' ⟦B'⟧)]] := by
  rw [IndexedStronglyNormalizing]
  let A'B'ki := A'ki.LE_weakening le |>.listApp B'isn.to_Kinding
  refine ⟨A'B'ki, ?_, ?_⟩
  · sorry
  · intro _ _ _ mst
...
theorem IndexedStronglyNormalizing.var (aKinΔ : [[a : K ∈ Δ]]) : [[Δ ⊢ SN K (a)]] := by
  ...
  | [[K₁ ↦ K₂]] =>
    ...
    constructor
    · intro B'isn
      apply IndexedStronglyNormalizing.not_lam_app _ _ A'ki B'isn le
      · intro Δ le K A'' mst
...
theorem StronglyNormalizing.of_Indexed (Aisn : [[Δ ⊢ SN K (A)]]) : [[Δ ⊢ SN(A)]] := by
  ...
  | arr K₁ =>
    ...
    let ⟨Asn, _⟩ := app_inversion <| StronglyNormalizing.of_Indexed <|
      Aisn.right (.var (.free a)) _ (.ext .refl anin (K := K₁)) |>.left <| .var .head
...
theorem weakening (Aisn : [[Δ ⊢ SN K (A)]]) (le : Δ ≤ Δ') : [[Δ' ⊢ SN K (A)]] := by
  ...
  | [[K₁ ↦ K₂]] =>
    ...
    intro B _ le'
    exact ⟨
      (Aisn.right _ _ (Trans.trans le le') |>.left ·),
      (Aisn.right _ _ (Trans.trans le le') |>.right ·)
    ⟩
  | [[L K']] =>
    rw [IndexedStronglyNormalizing] at Aisn ⊢
...
theorem preservation (Aisn : [[Δ ⊢ SN K (A)]]) (Ast : [[Δ ⊢ A -> B]]) : [[Δ ⊢ SN K (B)]] := by
  ...
  case arr =>
    ...
    constructor
    · intro B'isn
      exact h B' Δ' le |>.left B'isn |>.preservation <| .appl <| Ast.LE_weakening le
    · intro B'isn
      exact h B' Δ' le |>.right B'isn |>.preservation <| .listAppl <| Ast.LE_weakening le
  case list =>
    let ⟨Aki, Asn, h⟩ := Aisn
    ...
    · intro B'isn
      ...
      | intro _ _ ih =>
        ...
        | appl A'st =>
          ...
          exact h.right _ _ le |>.left B'isn
        | appr B'st => exact ih _ B'st <| preservation B'isn B'st
    · intro B'isn
      induction StronglyNormalizing.of_Indexed B'isn with
      | intro _ _ ih =>
        apply preservation_rev .listApp _ <| .listApp (Aki.LE_weakening le)
          B'isn.to_Kinding
        intro _ st
        cases st with
        | listAppList Aki' =>
          cases Aki.LE_weakening le |>.deterministic Aki'
          rw [IndexedStronglyNormalizing]
          let ⟨B'ki, h'⟩ := B'isn.to_Kinding.inv_list
          refine ⟨?_, ?_⟩
          · apply Kinding.list (Aki'.app <| B'ki · ·)
            split at h'
            · case isTrue h'' => exact .inr h''
            · case isFalse => exact .inl h'
          · have : ∀ {p q}, (q → p) → q → p ∧ q := fun a b => ⟨a b, b⟩
            ...
            generalize ABseq : Type.list .. = ABs at mst
            -- generalize ABseq : [:_].map .. = ABs, AB'seq : [:_].map _ = AB's, K?eq : Option.someIf .. = K? at mst
            induction mst using Relation.ReflTransGen.head_induction_on with
            | refl => sorry
            | head st _ ih =>
              -- cases ABseq
              ...
              -- apply ih
              sorry
            -- rcases mst.cases_head with eq | ⟨_, st, mst'⟩
            -- · sorry
            -- ·
        ...
        | listAppl A'st =>
          specialize h _ (A'st.LE_strengthening le Aki)
          rw [IndexedStronglyNormalizing] at h
          exact h.right _ _ le |>.right B'isn
        | listAppr B'st => exact ih _ B'st <| preservation B'isn B'st
  | [[L K']] =>
    rw [IndexedStronglyNormalizing]
    refine ⟨Aki, ?_, ?_⟩
    · apply StronglyNormalizing.preservation_rev
...
judgement_syntax Δ " ⊢ " A₀ " ⟦⟦" B₀ "⟧⟧" " ⇝ " A₁ " ⟦⟦" B₁ "⟧⟧" : CompShift

judgement CompShift where

Δ ⊢ B : K₁ ↦ K₂
────────────────────────────────────────────── mk
Δ ⊢ A ⟦⟦B ⟦C⟧ ⟧⟧ ⇝ (λ a : K₁. A (B a$0)) ⟦⟦C⟧⟧

judgement_syntax Δ " ⊢ " A₀ " ⟦⟦" B₀ "⟧⟧" " ⇝* " A₁ " ⟦⟦" B₁ "⟧⟧" : CompShifts
...
theorem preservation (A₀B₀ki : [[Δ ⊢ A₀ ⟦B₀⟧ : K]]) (cs : [[Δ ⊢ A₀ ⟦⟦B₀⟧⟧ ⇝ A₁ ⟦⟦B₁⟧⟧]])
  : [[Δ ⊢ A₁ ⟦B₁⟧ : K]] := by
  let .mk B₀₀ki := cs
  let .listApp A₀ki (.listApp B₀₀ki' B₀₁ki) := A₀B₀ki
  cases B₀₀ki.deterministic B₀₀ki'
  apply Kinding.listApp _ B₀₁ki
  apply Kinding.lam Δ.typeVarDom
  intro a anin
  simp [Type.TypeVar_open, A₀ki.TypeVarLocallyClosed_of.TypeVar_open_id,
        B₀₀ki.TypeVarLocallyClosed_of.TypeVar_open_id]
  exact .app (A₀ki.LE_weakening (.ext .refl anin)) <|
    .app (B₀₀ki.LE_weakening (.ext .refl anin)) <| .var .head

theorem stepl (cs : [[Δ ⊢ A₀ ⟦⟦B₀⟧⟧ ⇝ A₁ ⟦⟦B₁⟧⟧]]) (A₀st : [[Δ ⊢ A₀ -> A₀']])
  (A₀lc : A₀.TypeVarLocallyClosed)
  : ∃ A₁', [[Δ ⊢ A₁ -> A₁']] ∧ [[Δ ⊢ A₀' ⟦⟦B₀⟧⟧ ⇝ A₁' ⟦⟦B₁⟧⟧]] := by
  let .mk B₀₀ki (B := B₀₀) (C := B₀₁) (K₁ := K₁) := cs
  refine ⟨[[λ a : K₁. A₀' (B₀₀ a$0)]], ?_, .mk B₀₀ki⟩
  apply SmallStep.lam Δ.typeVarDom
  intro a anin
...
theorem stepr (cs : [[Δ ⊢ A₀ ⟦⟦B₀⟧⟧ ⇝ A₁ ⟦⟦B₁⟧⟧]]) (B₁st : [[Δ ⊢ B₁ -> B₁']])
  ...
  let .mk B₀₀ki (B := B₀₀) (C := B₀₁) (K₁ := K₁) := cs
  exact ⟨_, .listAppr B₁st, .mk B₀₀ki⟩

end CompShift

namespace CompShifts

theorem preservation (A₀B₀ki : [[Δ ⊢ A₀ ⟦B₀⟧ : K]])
  : [[Δ ⊢ A₀ ⟦⟦B₀⟧⟧ ⇝* A₁ ⟦⟦B₁⟧⟧]] → [[Δ ⊢ A₁ ⟦B₁⟧ : K]]
  | .refl => A₀B₀ki
  | .tail css cs (b := (_, _)) => by
    let .mk ki := cs
    exact CompShift.preservation (preservation A₀B₀ki css) (.mk ki)

theorem stepl (A₀st : [[Δ ⊢ A₀ -> A₀']]) (A₀lc : A₀.TypeVarLocallyClosed)
  (B₀lc : B₀.TypeVarLocallyClosed)
  : [[Δ ⊢ A₀ ⟦⟦B₀⟧⟧ ⇝* A₁ ⟦⟦B₁⟧⟧]] → ∃ A₁', [[Δ ⊢ A₁ -> A₁']] ∧ [[Δ ⊢ A₀' ⟦⟦B₀⟧⟧ ⇝* A₁' ⟦⟦B₁⟧⟧]]
  | .refl => ⟨_, A₀st, .refl⟩
  | .tail cs' cs (b := (_, _)) => by
    simp only at cs
    let .mk ki (B := B') := cs
    let ⟨_, st', cs''⟩ := stepl A₀st A₀lc B₀lc cs'
    let ⟨_, st'', cs'''⟩ := CompShift.stepl (.mk ki) st' sorry (B₁ := B₁)
    exact ⟨_, st'', .tail cs'' cs'''⟩

theorem stepr (B₁st : [[Δ ⊢ B₁ -> B₁']])
  : [[Δ ⊢ A₀ ⟦⟦B₀⟧⟧ ⇝* A₁ ⟦⟦B₁⟧⟧]] → ∃ B₀', [[Δ ⊢ B₀ -> B₀']] ∧ [[Δ ⊢ A₀ ⟦⟦B₀'⟧⟧ ⇝* A₁ ⟦⟦B₁'⟧⟧]]
  | .refl => ⟨_, B₁st, .refl⟩
  | .tail css cs (b := (_, _)) => by
    let .mk ki (A := A) := cs
    rw [← CompShifts] at css
    let ⟨_, st', cs'⟩ := CompShift.stepr (.mk ki (A := A)) B₁st
    let ⟨_, st'', css'⟩ := stepr st' css
    exact ⟨_, st'', .tail css' cs'⟩
...
theorem listApp (Aki : [[Δ ⊢ A : K₁ ↦ K₂]]) (Asn : [[Δ ⊢ SN(A)]]) (Bisn : [[Δ ⊢ SN L K₁ (B)]])
  (h : ∀ B' Δ', Δ ≤ Δ' → [[Δ' ⊢ SN K₁ (B')]] → [[Δ' ⊢ SN K₂ (A B')]]) (Δwf : [[⊢ Δ]])
  : [[Δ ⊢ SN L K₂ (A ⟦B⟧)]] := by
  let Bsn : [[Δ ⊢ SN(B)]] := .of_Indexed Bisn
...
theorem of_Kinding' (δsat : [[δ ⊨ Δ ≤ Δ']]) (Aki : [[Δ ⊢ A : K]]) (Δwf : [[⊢ Δ]]) (Δ'wf : [[⊢ Δ']])
  ...
  | lam I A'ki ih =>
    ...
    · intro Bisn
      let .lam I' A'ki' := Aki
      apply lam_app (A'.freeTypeVars ++ δ.dom ++ δ.freeTypeVars ++ I ++ I' ++ Δ.typeVarDom ++
          Δ'.typeVarDom ++ Δ''.typeVarDom) _ _ Bisn _ <| Δ'wf.LE_weakening le
      · intro a anin
      ...
      · intro a anin
        ...
        exact LE_weakening
          (of_Indexed <| ih _ aninI δsat' (Δwf.typeVarExt aninΔ) (Δ'wf.typeVarExt aninΔ'))
      ...
      · let ⟨a, anin⟩ := A'.freeTypeVars ++ (δ A').freeTypeVars ++ δ.dom ++ δ.freeTypeVars ++ I ++
        ...
        exact ih a aninI (δ := δ.ext B a) (δsat.weakening le |>.ext aninδd aninδfv Bisn)
          (Δwf.typeVarExt aninΔ) (Δ'wf.LE_weakening le)
    · intro Bisn
      apply listApp (Aki.LE_weakening le) _ Bisn _ (Δ'wf.LE_weakening le)
      sorry
      sorry
  | app A'ki Bki ih₀ ih₁ =>
    specialize ih₀ δsat
    specialize ih₁ δsat
    rw [Subst.apply]
    rw [IndexedStronglyNormalizing] at ih₀
    let ⟨_, h⟩ := ih₀ Δwf Δ'wf
    exact h _ _ .refl |>.left <| ih₁ Δwf Δ'wf
  | scheme I A'ki ih =>
    rename_i Δ _ _
    rename «Type» => A'
    rename Kind => K
    ...
    let ⟨aninI, aninδ⟩ := List.not_mem_append'.mp aninIδ
    let δsat' := δsat.TypeVar_ext aninδ aninΔ' (K := K)
    replace ih := of_Indexed <| ih a aninI δsat' (Δwf.typeVarExt aninΔ) (Δ'wf.typeVarExt aninΔ')
    rw [SubstSatisfies.apply_TypeVar_open_comm δsat' aninδ (A'ki a aninI)] at ih
    exact ih
  | arr A'ki Bki ih₀ ih₁ =>
    rw [IndexedStronglyNormalizing, Subst.apply]
    refine ⟨.arr (δsat.preservation A'ki) (δsat.preservation Bki), ?_⟩
    exact arr (of_Indexed (ih₀ δsat Δwf Δ'wf)) (of_Indexed (ih₁ δsat Δwf Δ'wf))
  | list A'ki h ih =>
    rename Nat → «Type» => A'
    rw [IndexedStronglyNormalizing]
    ...
    · intro A'' n b mst i mem
      ...
      cases neq
      specialize mst' i mem
      specialize ih i mem δsat Δwf Δ'wf
      simp only at mst'
      rw [Function.comp] at mst'
      rw [Range.eq_of_mem_of_map_eq Aseq i mem]
      generalize δ _ = A'''', A''' i = A''''' at mst' ih
      clear mst Aseq eq A'ki
      induction mst' using Relation.ReflTransGen.head_induction_on with
      | refl => exact ih
      | head st _ ih' => exact ih' <| preservation ih st
  | listApp A'ki B'ki ih₀ ih₁ =>
    ...
    rw [IndexedStronglyNormalizing] at ih₀
    let ⟨_, h⟩ := ih₀ Δwf Δ'wf
    exact h _ _ .refl |>.right <| ih₁ Δwf Δ'wf
  | prod A'ki ih =>
    rename «Type» => A'
    rw [IndexedStronglyNormalizing, Subst.apply]
    refine ⟨.prod <| δsat.preservation A'ki, ?_⟩
    clear A'ki
    replace ih := of_Indexed <| ih δsat Δwf Δ'wf
    generalize δ A' = A'' at ih
    induction ih with
    | intro _ _ ih' =>
      constructor
      intro _ st
      let .prod st' := st
      rw [StronglyNormalizing] at ih'
      apply ih'
      rw [Rel.inv, flip]
      exact st'
  | sum A'ki ih =>
    rename «Type» => A'
    rw [IndexedStronglyNormalizing, Subst.apply]
    refine ⟨.sum <| δsat.preservation A'ki, ?_⟩
    clear A'ki
    replace ih := of_Indexed <| ih δsat Δwf Δ'wf
    generalize δ A' = A'' at ih
    induction ih with
...
theorem of_Kinding (Aki : [[Δ ⊢ A : K]]) (Δwf : [[⊢ Δ]]) : IndexedStronglyNormalizing Δ K A := by
  rw [← Subst.apply_empty_id (A := A)]
  apply of_Kinding' _ Aki Δwf Δwf
  refine ⟨.nil, ?_⟩
  intro a K' aK'inΔ
