...
theorem preserve_lc (red: [[ Δ ⊢ A ≡> B ]]): A.TypeVarLocallyClosed → B.TypeVarLocallyClosed := by
  ...
  case listAppComp lcA _ _ ihA ihB =>
    intro lc
    let .listApp Alc Blc := lc
    let .listApp (.lam B₀'lc) B₁lc := ihB Blc
    exact .listApp (.lam (.app (ihA Alc).weaken B₀'lc)) B₁lc
  all_goals
...
theorem preserve_lc_rev (red: [[ Δ ⊢ A ≡> B ]]): B.TypeVarLocallyClosed → A.TypeVarLocallyClosed := by
  ...
  case listAppComp Alc _ _ ihA ihB =>
    intro lc
    let .listApp (.lam (.app _ B₀'lc)) B₁'lc := lc
    let Blc := ihB <| .listApp (.lam B₀'lc) B₁'lc
    exact .listApp Alc Blc
  all_goals
    set_option aesop.dev.statefulForward false in
    aesop (add safe forward modus_ponens_open, safe forward Std.Range.mem_of_mem_toList, safe TypeVarLocallyClosed, unsafe cases TypeVarLocallyClosed)
...
theorem weakening_type' (red: [[ Δ, Δ' ⊢ A ≡> B ]]) (freshΔ: a ∉ Δ.typeVarDom) : [[ Δ, a: K, Δ' ⊢ A ≡> B ]] := by
  ...
  · case lamListApp Δ_ _ _ _ _ _ _ _ Alc ihA ihB =>
    subst Δ_
    exact lamListApp (ihA freshΔ rfl) (ihB · · freshΔ rfl) Alc
  . case listAppId Δ_ _ _ _ _ Bki _ _ ihA ihB =>
    subst Δ_
    refine .listAppId ?_ (ihA freshΔ rfl) (ihB freshΔ rfl)
    rw [<- Environment.append_type_assoc]
    exact Kinding.weakening_r' (fresh := by simp_all [Environment.typeVarDom]) Bki
  ...
  . case scheme I Δ_ K' A B red ih =>
    ...
    specialize @ih a' (by simp_all) Δ (Δ'.typeExt a' K')
    simp_all [Environment.append]
  . case listApp Δ_ _ _ _ _ _ _ ihA ihB =>
    subst Δ_
    exact .listApp (ihA freshΔ rfl) (ihB freshΔ rfl)
  . case listAppComp Δ_ _ _ _ _ _ Alc _ _ ihA ihB =>
    subst Δ_
    exact .listAppComp Alc (ihA freshΔ rfl) (ihB freshΔ rfl)
...
theorem weakening_term' (red: [[ Δ, Δ' ⊢ A ≡> B ]]) : [[ Δ, x: T, Δ' ⊢ A ≡> B ]] := by
  ...
  . case listAppId Δ_ _ _ _ _ Bki _ _ ihA ihB =>
    subst Δ_
    refine .listAppId ?_ (ihA rfl) (ihB rfl)
...
theorem TypeVar_drop_of_not_mem_freeTypeVars (Apr : [[Δ, a : K, Δ' ⊢ A ≡> B]])
  ...
  | listAppId B'ki A'pr B'pr =>
    simp [freeTypeVars] at aninA
    let ⟨aninA', aninB'⟩ := aninA
    exact listAppId (B'ki.TypeVar_drop_of_not_mem_freeTypeVars aninB')
  ...
  | listApp A'pr B'pr =>
    ...
    exact listApp (A'pr.TypeVar_drop_of_not_mem_freeTypeVars aninA')
      (B'pr.TypeVar_drop_of_not_mem_freeTypeVars aninB')
  | listAppComp A'lc A'pr B'pr =>
    simp [freeTypeVars] at aninA
    let ⟨aninA', aninB'⟩ := aninA
    apply listAppComp A'lc (A'pr.TypeVar_drop_of_not_mem_freeTypeVars aninA') <|
      B'pr.TypeVar_drop_of_not_mem_freeTypeVars aninB'
  | prod A'pr =>
...
theorem subst_out' {A T T' : «Type»} (wf: [[ ⊢ Δ, a: K, Δ' ]]) (red : [[ Δ, a: K, Δ' ⊢ T ≡> T' ]]) (kindA: [[ Δ ⊢ A: K ]]) : [[ Δ, Δ'[A/a] ⊢ T[A/a] ≡> T'[A/a] ]] := by
  ...
  . case listAppId Δ_ _ _ _ _ Bki _ _ ihA ihB =>
    subst Δ_
    refine .listAppId ?_ (ihA wf kindA rfl) (ihB wf kindA rfl)
    apply Kinding.subst' (K := K) <;> simp_all
  ...
  . case listAppComp Δ_ _ _ _ _ _ Alc _ _ ihA ihB =>
    subst Δ_
    exact .listAppComp (Alc.TypeVar_subst kindA.TypeVarLocallyClosed_of) (ihA wf kindA rfl)
      (ihB wf kindA rfl)
...
theorem subst_all' {A B T: «Type»} (wf: [[ ⊢ Δ, a: K, Δ' ]]) (red1: [[ Δ ⊢ A ≡> B ]]) (red2: [[ Δ, a: K, Δ' ⊢ T ≡> T' ]]) (kindA: [[ Δ ⊢ A: K ]]) (lcT: T.TypeVarLocallyClosed): [[ Δ, Δ'[A/a] ⊢ T[A/a] ≡> T'[B/a] ]] := by
  ...
  . case listAppId Δ_ _ _ _ _ Bki _ _ ihA ihB =>
    subst Δ_
    let .listApp Alc Blc := lcT
    simp [«Type».TypeVar_subst] at ihA ihB ⊢
    refine .listAppId ?_ (ihA wf red1 kindA Alc rfl) (ihB wf red1 kindA Blc rfl)
    exact Bki.subst' wf kindA
  ...
  . case listApp Δ_ _ _ _ _ _ _ ihA ihB =>
    ...
    cases lcT
    simp_all [«Type».TypeVar_subst]
    exact listApp (ihA wf red1 kindA rfl) (ihB wf red1 kindA rfl)
  . case listAppComp Δ_ _ _ _ _ _ Alc _ _ ihA ihB =>
    subst Δ_
    let .listApp Alc Blc := lcT
    simp [«Type».TypeVar_subst] at ihA ihB ⊢
    exact listAppComp (Alc.TypeVar_subst kindA.TypeVarLocallyClosed_of) (ihA wf red1 kindA Alc rfl)
      (ihB wf red1 kindA Blc rfl)
  . case prod =>
    cases lcT
    simp_all [«Type».TypeVar_subst]
    constructor; simp_all
...
theorem preservation (red: [[ Δ ⊢ A ≡> B ]]) (wf: [[ ⊢ Δ ]]) (k: [[ Δ ⊢ A: K ]]): [[ Δ ⊢ B: K ]] := by
  ...
  case lamListApp Δ A A' n B B' redA redB Alc ihA ihB =>
    ...
    . case succ n =>
      ...
      exact kA'.app kB'
  case listAppComp Δ _ _ _ _ _ Alc AA' _ ihA ihB =>
    let .listApp Aki Bki := k
    let A'ki := ihA wf Aki
    let .listApp (.lam I B₀'ki) B₁'ki := ihB wf Bki
    refine .listApp (.lam (I ++ Δ.typeVarDom) ?_) B₁'ki
    intro a anin
    let ⟨aninI, aninΔ⟩ := List.not_mem_append'.mp anin
    simp [Type.TypeVar_open]
    rw [AA'.preserve_lc Alc |>.TypeVar_open_id]
    refine .app (A'ki.weakening (wf.typeVarExt aninΔ) (Δ' := .typeExt .empty ..) (Δ'' := .empty)) ?_
    exact B₀'ki a aninI
  ...
  case listAppId Bki _ _ ihA ihB =>
    let .listApp Aki Bki := k
    let .lam I idki := ihA wf Aki
    let ⟨a, anin⟩ := I.exists_fresh
    specialize idki a anin
    simp [Type.TypeVar_open] at idki
    let .var .head := idki
    exact ihB wf Bki
