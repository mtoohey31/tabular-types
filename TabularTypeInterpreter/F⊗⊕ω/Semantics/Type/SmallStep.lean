...
open «Type» in
theorem TypeVar_subst_var {a' : TypeVarId} (Ast : [[Δ, a : K, Δ' ⊢ A -> B]])
  ...
  | lamApp A'ki B'ki A'v B'v =>
    ...
    let A'v' := A'v.TypeVar_subst_var (a := a) (a' := a')
    rw [TypeVar_subst] at A'v' ⊢
    apply lamApp _ _ A'v' B'v.TypeVar_subst_var
    sorry
    sorry
    sorry
  | listAppList ne _ A'v B'v (K₁ := K₁) (K₂ := K₂) =>
    rename_i A' _ B' _
    ...
    apply listAppList (ne_preservation ne) _ A'v.TypeVar_subst_var (B'v · · |>.TypeVar_subst_var) (K₁ := K₁) (K₂ := K₂)
    sorry
  | listAppId A'ki A'v =>
    rw [TypeVar_subst, TypeVar_subst, TypeVar_subst, if_neg nofun]
    apply listAppId _ A'v.TypeVar_subst_var
    sorry
  | listAppComp ne A₁ki A₀v A₁B'v (K₂ := K₂) =>
    ...
    apply listAppComp (ne_preservation ne) _ A₀v.TypeVar_subst_var A₁B'v (K₂ := K₂)
    sorry
  | lam I A'v =>
    rw [TypeVar_subst, TypeVar_subst]
    apply lam <| a :: I
...
theorem progress (Aki : [[Δ ⊢ A : K]]) : A.IsValue ∨ ∃ B, [[Δ ⊢ A -> B]] := match A, Aki with
  ...
  | .lam K A', .lam I A'ki => by
    ...
    | .inr ⟨A'', A'st⟩ =>
      ...
      exact TypeVar_subst_var A'st (Δ' := .empty)
...
open «Type» in
theorem TypeVar_subst_var_preservation_of_not_mem_freeTypeVars {a' : TypeVarId}
  (Ast : [[Δ, a : K ⊢ A ->* B]]) : [[Δ, a' : K ⊢ A[a' / a] ->* B[a' / a] ]] := by
  induction Ast with
  | refl => exact refl
  | step st _ ih => exact step (st.TypeVar_subst_var (Δ' := .empty)) ih
...
theorem TypeVar_open_swap (Amst : [[Δ, a' : K ⊢ A^a' ->* A']]) (Alc : A.TypeVarLocallyClosed 1)
  (a'ninA : a' ∉ A.freeTypeVars) : [[Δ, a : K ⊢ A^a ->* (\a'^A')^a]] := by
  ...
  | step st =>
    ...
    have : A'.TypeVar_open a' = (A'.TypeVar_open a).TypeVar_subst a (.var a') := by
      rw [Type.Type_open_var, Type.Type_open_var,
          ...
          if_pos rfl, Type.TypeVar_subst_id_of_not_mem_freeTypeVars aninA']
    rw [this]
    exact SmallStep.TypeVar_subst_var st (Δ' := .empty)
  | symm est ih =>
...
theorem SmallStep.Type_open_out (Bst : [[Δ ⊢ B -> B']])
  ...
  | .var _ =>
    ...
    · case isTrue h =>
      ...
      exact .step <| Bst.weakening sorry (Δ'' := .empty)
    · case isFalse h =>
...
theorem MultiSmallStep.Type_open_out (Bst : [[Δ ⊢ B ->* B']])
  ...
  | step st _ ih =>
    ...
    exact .trans (st.Type_open_out Aki ΔaΔ'wf aninA Bki) <| ih <| st.preservation Bki

theorem SmallStep.TypeVar_subst_in (Ast : [[Δ, a : K, Δ' ⊢ A -> A']]) (Aki : [[Δ, a : K, Δ' ⊢ A : K']])
  (Δwf : [[⊢ Δ, a : K, Δ']]) (Bki : [[Δ ⊢ B : K]])
  : [[Δ, Δ' ⊢ A[B / a] <->* A'[B / a] ]] := open «Type» in by
  match A, Ast, Aki with
  | .lam K'' (.app A'' _), .eta I _ A''av, .lam I' A''aki =>
    simp [Type.TypeVar_subst]
    let ⟨a, anin⟩ := A''.freeTypeVars ++ I ++ I' |>.exists_fresh
    let ⟨aninA''I, aninI'⟩ := List.not_mem_append'.mp anin
    let ⟨aninA'', aninI⟩ := List.not_mem_append'.mp aninA''I
    let .app A''lc _ := A''av a aninI |>.TypeVarLocallyClosed_of
  ...
  | .app (.lam K'' A'') B', .lamApp .., .app (.lam I A''ki) B'ki =>
    ...
    calc
      ...
      [[(λ a : K''. \a'^A''') B'' <->* (\a'^A''')^^B'']] := by
        refine .lamApp I ?_ B''v
        intro a'' a''nin
        rw [Type_open_var, A'''v.TypeVarLocallyClosed_of.Type_open_TypeVar_close_eq_TypeVar_subst]
        exact A'''v.TypeVar_subst_var
      [[(\a'^A''')^^B'' <->* A''[B / a]^^B'[B / a] ]] := by
        apply EqSmallStep.symm
        rw [Bki.TypeVarLocallyClosed_of.TypeVar_open_TypeVar_subst_comm a'ne.symm] at A''ki' A'''mst
        rw [← A'''v.TypeVarLocallyClosed_of.TypeVar_open_TypeVar_close_id
                (a := a') (n := 0)] at A'''mst
        -- TODO: Can't figure out the induction here... might need something along the lines of the
        -- normalization proof to be able to show this is well-founded.
        sorry
        -- exact A'''mst.Type_open B''mst A''ki' (Δ' := .empty) sorry a'ninA''
    ...
    · case pos h =>
      ...
      calc
        ...
        [[{</ B''@i // i in [:n] />} <->* {</ (λ a : K₁. a$0) B''@i // i in [:n] />}]] := by
          apply EqSmallStep.symm
          apply EqSmallStep.list fun i mem =>
            .app .id <| B''vB''mst i mem |>.right.preservation (B'ki' i mem |>.subst' Δwf Bki)
          intro i mem
          have : B'' i = Type.Type_open (.var (.bound 0)) (B'' i) := by
            rw [Type.Type_open, if_pos rfl]
          rw (occs := .pos [2]) [this]
          exact .lamApp [] (fun _ _ => by rw [TypeVar_open, if_pos rfl]; exact .var)
            (B''vB''mst i mem |>.left)
  ...
  | .lam K'' A'', .lam I A''st, .lam I' A''ki =>
    ...
    specialize A''ki a'' a''ninI'
    rw [← Environment.append] at A''ki
    exact (A''st a'' a''ninI).TypeVar_subst_in A''ki (Δwf.typeVarExt a''ninΔ) Bki
...
theorem SmallStep.Type_open_in
  ...
  : EqSmallStep [[Δ, Δ']] (A.Type_open B n) (A'.Type_open B n) := by
...
theorem MultiSmallStep.Type_open
  (Amst : MultiSmallStep [[Δ, a : K, Δ']] (A.TypeVar_open a n) (Type.TypeVar_open A' a n)) (Bmst : [[Δ ⊢ B ->* B']])
  (Aki : Kinding [[Δ, a : K, Δ']] (Type.TypeVar_open A a n) K') (Δwf : [[⊢ Δ, a : K, Δ']])
  (aninA : a ∉ A.freeTypeVars) (aninA' : a ∉ A'.freeTypeVars) (Bki : [[Δ ⊢ B : K]])
  : EqSmallStep [[Δ, Δ']] (A.Type_open B n) (A'.Type_open B' n) := by
...
theorem EqSmallStep.lamApp (I : List TypeVarId) (Aki : ∀ a ∉ I, [[Δ, a : K₁ ⊢ A^a : K₂]])
  ...
  let ⟨(a : TypeVarId), anin⟩ := A.freeTypeVars ++ I ++ Δ.typeVarDom |>.exists_fresh
  let ⟨aninAI, aninΔ⟩ := List.not_mem_append'.mp anin
  let ⟨aninA, aninI⟩ := List.not_mem_append'.mp aninAI
  let Alc := Aki a aninI |>.TypeVarLocallyClosed_of.TypeVar_close_inc (a := a)
  rw [Type.TypeVar_close_TypeVar_open_eq_of_not_mem_freeTypeVars aninA] at Alc
  let ⟨A', A'v, A'mst⟩ := MultiSmallStep.normalization <| Aki a aninI
  ...
  · case pos h =>
    ...
    let A''aki := A'mst.preservation (Aki a aninI)
    calc
      [[Δ ⊢ (λ a : K₁. A) B <->* (λ a : K₁. A'' a$0) B']] := by
        apply app (.lam I Aki) _ B'mst
        apply lam I _ Alc
        intro a' a'nin
        simp [Type.TypeVar_open, A''lc.TypeVar_open_id]
        let A'mst' := A'mst.TypeVar_subst_var_preservation_of_not_mem_freeTypeVars (a' := a')
      ...
      [[Δ ⊢ (λ a : K₁. A'' a$0) B' <->* A'' B']] := by
        ...
        · apply step <| .eta (K₂ := K₂) [] ..
          ...
          · intro a' a'nin
            ...
            exact A'v
      [[Δ ⊢ A'' B' <->* A^^B]] := by
        clear A''aki A''lc A'v anin B'v B'mst aninAI
          ...
          case app A''''' =>
            ...
            case var =>
              ...
              · case isTrue h =>
                cases h
                simp [Type.Type_open]
                sorry -- TODO: get locally closed out of aninA''
              · case isFalse h =>
                cases eq'
                simp [Type.freeTypeVars] at aninA
            all_goals nomatch eq'
        ...
        | step st _ ih =>
          ...
          apply trans <| ih _ Type.not_mem_freeTypeVars_TypeVar_close
            (A₁lc.TypeVar_close_inc (a := a)) _ aninA'' A₁lc.TypeVar_open_TypeVar_close_id rfl
          · symm
            apply SmallStep.Type_open_in _ (Aki a aninI) aninA Type.not_mem_freeTypeVars_TypeVar_close
              (Δwf.typeVarExt aninΔ) Bki (Δ' := .empty)
            rw [A₁lc.TypeVar_open_TypeVar_close_id]
            exact st
          · intro a' a'nin
            let f := st.preservation (Aki a aninI)
            sorry
  · case neg h =>
    sorry
    -- apply step <| .lamApp sorry sorry _ Bv
    -- refine .lam I Av ?_
    -- apply not_exists.mp at h
    -- intro A'' eq
    -- specialize h A''
...
-- TODO: This measure doesn't work, cause the inside might've
private
def listApp_depth : «Type» → Nat
  | .listApp _ A => listApp_depth A + 1
  | _ => 0
    ...
    calc
      ...
      [[Δ ⊢ (λ a : K₁. A₁ a$0) ⟦B'⟧ <->* (λ a : K₁. (λ a : K₂. a$0) (A₁ a$0)) ⟦B'⟧]] := by
        ...
        · apply lam Δ.typeVarDom _ <| .app (A₁lc.weaken (n := 1)) <| .var_bound Nat.one_pos
          ...
          apply lamApp [] (fun _ _ => by rw [Type.TypeVar_open, if_pos rfl]; exact .var .head) _
            Δa'wf
          exact .app
      ...
      | .listAppList ne' _ A₁v B''v (B := B'') (n := n) =>
        ...
        calc
          ...
          [[Δ ⊢ {</ A₀ (A₁ B''@i) // i in [:n] />} <->* {</ (λ a : K₁. A₀ (A₁ a$0)) B''@i // i in [:n] />}]] := by
            ...
            rw [this]
            symm
            apply lamApp Δ.typeVarDom _ (B'ki' i mem) Δwf (K₂ := K₃)
            intro a anin
            simp [Type.TypeVar_open, A₀lc.TypeVar_open_id, A₁lc.TypeVar_open_id]
            let Δawf := Δwf.typeVarExt anin (K := K₁)
            exact .app (A₀ki.weakening Δawf (Δ' := .typeExt .empty ..) (Δ'' := .empty)) <|
        ...
        calc
          ...
          [[Δ ⊢ (λ a : K₀. A₀ ((λ a : K₀. A₁ (B₀ a$0)) a$0)) ⟦B₁⟧ <->* (λ a : K₀. A₀ (A₁ (B₀ a$0))) ⟦B₁⟧]] := by
            ...
            · apply lam Δ.typeVarDom _ <| .app (A₀lc.weaken (n := 1)) <|
              ...
              rw [this]
              apply lamApp (a :: Δ.typeVarDom) _ (.var .head) Δawf (K₂ := K₂)
              intro a' a'nin
              simp [Type.TypeVar_open, A₁lc.TypeVar_open_id, B₀lc.TypeVar_open_id]
          ...
          [[Δ ⊢ (λ a : K₀. A₀ (A₁ (B₀ a$0))) ⟦B₁⟧ <->* (λ a : K₀. (λ a : K₁. A₀ (A₁ a$0)) (B₀ a$0)) ⟦B₁⟧]] := by
            ...
            · apply lam Δ.typeVarDom _ <| .app (A₀lc.weaken (n := 1)) <|
              ...
              have : [[A₀ (A₁ (B₀ a))]] = [[((A₀ (A₁ a$0))^^(B₀ a))]] := by
                simp [Type.Type_open, A₀lc.Type_open_id, A₁lc.Type_open_id]
              rw [this]
              apply lamApp (a :: Δ.typeVarDom) _ _ Δawf (K₂ := K₃)
              · intro a' a'nin
                simp [Type.TypeVar_open, A₀lc.TypeVar_open_id, A₁lc.TypeVar_open_id]
                let Δaa'wf := Δawf.typeVarExt a'nin (K := K₁)
                exact .app (A₀ki.weakening Δaa'wf (Δ' := [[ε, a : K₀, a' : K₁]]) (Δ'' := .empty)) <|
          ...
          calc
            [[Δ ⊢ (λ a : K₀. (λ a : K₁. a$0) (B₀ a$0)) ⟦B₁⟧ <->* (λ a : K₀. B₀ a$0) ⟦B₁⟧]] := by
              ...
              · apply lam Δ.typeVarDom _ <| .app (B₀lc.weaken (n := 1)) <| .var_bound Nat.one_pos
                ...
                rw (occs := .pos [2]) [this]
                let Δawf := Δwf.typeVarExt anin (K := K₀)
                apply lamApp [] (fun _ _ => by rw [Type.TypeVar_open, if_pos rfl]; exact .var .head)
                  _ Δawf
                exact .app (B₀ki.weakening Δawf (Δ' := .typeExt .empty ..) (Δ'' := .empty)) <|
                  .var .head
            [[Δ ⊢ (λ a : K₀. B₀ a$0) ⟦B₁⟧ <->* B₀ ⟦B₁⟧]] :=
...
theorem of_EquivalenceI (equ : [[Δ ⊢ A ≡ᵢ B]]) (Aki : [[Δ ⊢ A : K]]) (Δwf : [[⊢ Δ]])
  ...
  --   calc
  --     [[Δ ⊢ (λ a : K'. A') B' <->* (λ a : K'. \a^A'') B'']] := by
  --       apply app (.lam I A'ki) _ B''mst
  ...
  --       exact A''mst.Type_open B''mst (A'ki a aninI) (Δ' := .empty) (Δwf.typeVarExt aninΔ) aninA'
  --         Type.not_mem_freeTypeVars_TypeVar_close B'ki
  | listAppList =>
    rename_i A' Δ n B' _
    let .listApp A'ki B'ki (K₁ := K₁) := Aki
    ...
    let B''ki i mem := B''vB''mst i mem |>.right.preservation <| B'ki' i mem
    by_cases ∃ K', A'' = [[λ a : K'. a$0]]
    · case pos h =>
      rcases h with ⟨_, rfl⟩
      cases A''mst.preservation A'ki
      calc
        [[Δ ⊢ A' ⟦{</ B'@i // i in [:n] />}⟧ <->* (λ a : K₁. a$0) ⟦{</ B''@i // i in [:n] />}⟧]] :=
          listApp A'ki A''mst <| list B'ki' (B''vB''mst · · |>.right)
        [[Δ ⊢ (λ a : K₁. a$0) ⟦{</ B''@i // i in [:n] />}⟧ <->* {</ B''@i // i in [:n] />}]] :=
          step <| .listAppId (.list B''ki) <| .list (B''vB''mst · · |>.left)
        [[Δ ⊢ {</ B''@i // i in [:n] />} <->* {</ (λ a : K₁. a$0) B''@i // i in [:n] />}]] := by
          apply list
          · intro i mem
            exact B''vB''mst i mem |>.right.preservation (B'ki' i mem)
          · intro i mem
            symm
            have : B'' i = Type.Type_open (.var (.bound 0)) (B'' i) := by
              rw [Type.Type_open, if_pos rfl]
            rw (occs := .pos [2]) [this]
            exact step <| .lamApp .id (B''ki i mem) .id <| B''vB''mst i mem |>.left
        [[Δ ⊢ {</ (λ a : K₁. a$0) B''@i // i in [:n] />} <->* {</ A' B'@i // i in [:n] />}]] :=
          symm <| list (.app A'ki <| B'ki' · ·)
            (app A'ki A''mst <| B''vB''mst · · |>.right)
    · case neg ne =>
      calc
        [[Δ ⊢ A' ⟦{</ B'@i // i in [:n] />}⟧ <->* A'' ⟦{</ B''@i // i in [:n] />}⟧]] :=
          listApp A'ki A''mst <| list B'ki' (B''vB''mst · · |>.right)
        [[Δ ⊢ A'' ⟦{</ B''@i // i in [:n] />}⟧ <->* {</ A'' B''@i // i in [:n] />}]] :=
          step <| .listAppList (not_exists.mp ne) (A''mst.preservation A'ki) A''v
            (B''vB''mst · · |>.left)
        [[Δ ⊢ {</ A'' B''@i // i in [:n] />} <->* {</ A' B'@i // i in [:n] />}]] :=
          symm <| list (.app A'ki <| B'ki' · ·) (app A'ki A''mst <| B''vB''mst · · |>.right)
  | listAppId A'ki =>
    rename_i Δ A' K'
    let ⟨A'', A''v, A''mst⟩ := MultiSmallStep.normalization A'ki
    calc
