...
nonterminal (tex pre := "\\targetpre", post := "\\targetpost") Environment, Δ :=
  ...
  | Δ ", " a " : " K                         : typeExt (id a)
  | Δ «,,» aK:sepBy(a " : " K, ",, ")        : multiTypeExt notex (id a) (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Environment.multiTypeExt #[Δ, aK])
  | Δ ", " x " : " A                         : termExt (id x)
  | Δ «,,,» xA:sepBy(x " : " A, ",,, ")      : multiTermExt notex (id x) (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Environment.multiTermExt #[Δ, xA])
  | "(" Δ ")"                                : paren notex (expand := return Δ)
  | Δ ", " Δ'                                : append notex (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Environment.append #[Δ, Δ'])
  | Δ " [" A " / " a "]"                     : subst notex (id a) (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Environment.TypeVar_subst #[Δ, a, A])
  | Δ " ! " Aa:sepBy("[" A " / " a "]", "!") : multi_subst notex (id a) (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Environment.TypeVar_multi_subst #[Δ, Aa]) (tex := s!"{Δ} \\, {Aa}")
