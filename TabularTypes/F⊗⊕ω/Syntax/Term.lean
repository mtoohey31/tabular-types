...
nonterminal (tex pre := "\\targetpre", post := "\\targetpost") Term, E, F :=
  ...
  | E F                               : app
  | "! " Es:sepBy(E, ", ") F          : multiApp notex (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Term.multi_app #[Es, F]) (tex := s!"{Es} \\, {F}")
  ...
  | E "^^^" a "#" n                   : TypeVar_multi_open notex (id a) (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Term.TypeVar_multi_open #[E, a, n]) (tex := E)
  | E "^^^" x "#" n                   : TermVar_multi_open notex (id a) (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Term.TermVar_multi_open #[E, x, n]) (tex := E)
  | E "^^^^" A "@@" i "#" n "/" a     : Type_multi_open notex (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Term.Type_multi_open #[E, A, n]) (tex := s!"{E}\\lottsepbycomprehension\{\\left[\{{A}}_\{{i}}/\{{a}}_\{{i}}\\right]}")
  | E "^^^^" F "@@" i "#" n "/" x     : Term_multi_open notex (expand := return .mkCApp `TabularTypes.«F⊗⊕ω».Term.Term_multi_open #[E, F, n]) (tex := s!"{E}\\lottsepbycomprehension\{\\left[\{{F}}_\{{i}}/\{{x}}_\{{i}}\\right]}")
