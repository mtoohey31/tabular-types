...
import TabularTypes.«F⊗⊕ω».Lemmas.Type.Kinding
...
theorem subst_rename' {a': TypeVarId}
  ...
  . case eta T' K' T'lc => exact .eta <| T'lc.TypeVar_subst (by simp_all)
...
theorem subst' {A T T' : «Type»} (equiv : [[ Δ, a: K, Δ' ⊢ T ≡ T' ]]) (Tlc: T.TypeVarLocallyClosed) (wf: [[ ⊢ Δ, a: K, Δ' ]]) (AkiK: [[ Δ ⊢ A: K ]]) : [[ Δ, Δ'[A/a] ⊢ T[A/a] ≡ T'[A/a] ]] := by
  ...
  . case eta T' K' T'lc => exact .eta <| T'lc.TypeVar_subst AkiK.TypeVarLocallyClosed_of
