...
macro_rules
  | `([[$Γᵢ:Lott.Symbol.TabularTypes.InstanceEnvironment; $Γc:Lott.Symbol.TabularTypes.ClassEnvironment; $Γ:Lott.Symbol.TabularTypes.TypeEnvironment ⊨ $ψ:Lott.Symbol.TabularTypes.Monotype]]) =>
...
def constraintSolvingTexElab : Lott.TexElab := fun profile ref stx => do
  let `(constraintSolving| $Γᵢ:Lott.Symbol.TabularTypes.InstanceEnvironment; $Γc:Lott.Symbol.TabularTypes.ClassEnvironment; $Γ:Lott.Symbol.TabularTypes.TypeEnvironment ⊨ $ψ:Lott.Symbol.TabularTypes.Monotype) := stx
  ...
  let ψ ← Lott.texElabSymbolOrJudgement `Lott.Symbol.TabularTypes.Monotype profile ref ψ
...
notex for noelab Γc; Γ ⊢ ρ₅ : R κ ⇝ A₅
notex for noelab ⊢ κ ⇝ K
notex for noelab Eₚ' := ⦅⦅π 0 E₂⦆ [a$0] ⦅⦅π 0 Fₗ⦆ [a$0] ⦅⦅π 0 π 2 E₅⦆ [a$0] x$0⦆⦆⦆
notex for noelab Eₚ := Λ a : K ↦ *. λ x : ⊗ (a$0 ⟦A₅⟧). Eₚ' ⦅⦅π 0 Fᵣ⦆ [a$0] ⦅⦅π 0 π 3 E₅⦆ [a$0] x$0⦆⦆
notex for noelab Eᵢ' := ⦅⦅π 1 E₂⦆ [a$0] [⊕ (a$0 ⟦A₅⟧)] ⦅λ x : ⊕ (a$0 ⟦A₀⟧). ⦅π 1 π 2 E₅⦆ [a$0] ⦅⦅π 1 Fₗ⦆ [a$0] x$0⦆⦆⦆
notex for noelab Eᵢ := Λ a : K ↦ *. Eᵢ' ⦅λ x : ⊕ (a$0 ⟦A₁⟧). ⦅π 1 π 3 E₅⦆ [a$0] ⦅⦅π 1 Fᵣ⦆ [a$0] x$0⦆⦆
─────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── containConcat
Γᵢ; Γc; Γ ⊨ ρ₂ ≲(μ) ρ₅ ⇝ (Eₚ, Eᵢ)
...
</ Γᵢ; Γc; Γ ⊨ ⟨</ ℓ@j ▹ τ@j // j in [:i] /> </ : κ // i = 0 || b />⟩ ⊙(N) ⟨ℓ@i ▹ τ@i⟩ ~ ⟨</ ℓ@k ▹ τ@k // k in [:i+1] /> </ : κ // b />⟩ ⇝ E@i // i in [:n] />
</ Γᵢ; Γc; Γ ⊨ ⟨</ ℓ@j ▹ τ@j // j in [:i+1] /> </ : κ // b />⟩ ⊙(N) ⟨</ ℓ@k ▹ τ@k // k in [i+1:n] /> : κ⟩ ~ ⟨</ ℓ@l ▹ τ@l // l in [:n] /> </ : κ // b />⟩ ⇝ E'@i // i in [:n] />
notex for noelab E'' := ! </ ⦅⦅xₛ$1 [⊗ { : * }] [A@i] [{</ A@j // j in [:i] /> </ : K // i = 0 || b />}] [{</ A@k // k in [:i+1] /> </ : K // b />}] [{</ A@l // l in [i+1:n] /> : K}] E@i⦆ E'@i⦆ () // i in [:n] /> xᵢ$0
notex for noelab F := Λ aₘ : (L K) ↦ *. λ xₛ : Aₛ. λ xᵢ : aₘ$0 { : K }. E''
───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────── ind (I₀ I₁ : List TypeVarId)
Γᵢ; Γc; Γ ⊨ Ind ⟨</ ℓ@i ▹ τ@i // i in [:n] /> </ : κ // b />⟩ ⇝ F
